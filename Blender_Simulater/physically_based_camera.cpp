...
namespace game {
    ...
    void PhysicallyBasedCamera::Update(const GLfloat l_new, const double delta_time) {
        ...
        iso_ = std::clamp(ComputeIso(aperture_, shutter_speed_, target_ev), kMinIso, kMaxIso);

        GLfloat evDiff = target_ev - ComputeEv(aperture_, shutter_speed_, iso_);
        aperture_ = std::clamp(aperture_ * std::powf(std::sqrt(2.0f), evDiff * 0.5f), kMinAperture, kMaxAperture);

        evDiff = target_ev - ComputeEv(aperture_, shutter_speed_, iso_);
        shutter_speed_ = std::clamp(shutter_speed_ * std::powf(2.0f, -evDiff), kMinShutterSpeed, kMaxShutterSpeed);

        const GLfloat l_max = (7800.0f / 65.0f) * (aperture_ * aperture_) / (iso_ * shutter_speed_);
